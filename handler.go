...
func newPeer(source *websocket.Conn, target *net.TCPConn) *peer {
	return &peer{
		source: source,
		target: target,
...
func (h WebsocketVncProxyHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if strings.ToLower(r.Header.Get("Upgrade")) != "websocket" {
		...
		LOG.Error("not websocket", "client", r.RemoteAddr)
		return
	}
	...
	peer.source.SetPongHandler(func(string) error {
		peer.source.SetReadDeadline(time.Now().Add(h.timeout))
		return nil
	})
	...
	for {
		...
		case bs, ok := <-peer.sendSource:
			...
			err := peer.source.WriteMessage(websocket.BinaryMessage, bs)
			if err != nil {
				LOG.Error("can't send data to websocket",
					"addr", r.RemoteAddr, "err", err)
			}
		...
		case <-ticker.C:
			peer.source.SetWriteDeadline(time.Now().Add(h.timeout))
			err := peer.source.WriteMessage(websocket.PingMessage, nil)
			if err != nil {
...
func (h WebsocketVncProxyHandler) readSource(p *peer) {
	p.source.SetReadLimit(h.maxMsgSize)
	p.source.SetReadDeadline(time.Now().Add(h.timeout))
	addr := p.source.RemoteAddr().String()

	for {
		_, msg, err := p.source.ReadMessage()
		if err != nil {
